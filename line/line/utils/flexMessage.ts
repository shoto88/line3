...
export function getStatusMessage(waitingNumber: number, currentNumber: number, averageWaitingTime: number) {
  const estimatedWaitingTime = (waitingNumber - currentNumber) * averageWaitingTime;
  const now = new Date();
